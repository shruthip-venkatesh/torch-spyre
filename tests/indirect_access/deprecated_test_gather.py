...
def test_gather_1d():
    ...
    def gather_fn(input, index):
        result_2d = torch.ops.spyre.indirect_gather(input, index)
    ...
    address_tensor = torch.tensor([0, 10, 20, 30], dtype=torch.int64)
    print("Index Tensor Shape:", address_tensor.shape)  # Should be torch.Size([4])

    input_tensor = input_tensor.reshape(64, 64).to("spyre")
    address_tensor = address_tensor.reshape(4, 32).to("spyre")
