    ...
    def specialized_paged_attn_kernel(q, k_pages, v_pages, page_indices, mask_tiles, scale):
        ...
        for i in range(num_blocks):
            ...
            scores = _indirect_matmul_mock(
                q, None, k_pages, page_idx_expanded,
                transform_b=lambda t: t.transpose(-2, -1)  # Don't unsqueeze, already has batch dim
            )
            ...
            if i == 0:
                ...
                tile_output = _indirect_matmul_mock(
                    tile_probs, None, v_pages, page_idx_expanded,
...
def test_simple_indirect_matmul():
    ...
    a = torch.randn(NUM_PAGES, PAGE_SIZE, NUM_HEADS, HEAD_SIZE, dtype=torch.float16, device="spyre")
    print(f"Tensor A Shape: {a.shape}")
    
    # 4D tensor B: [batch, num_matrices, rows, cols]
    # Shape: [2, 3, 8, 64] - must match for matmul (COLS_A == ROWS_B)
    b = torch.randn(NUM_PAGES, PAGE_SIZE, NUM_HEADS, HEAD_SIZE, dtype=torch.float16, device="spyre")
    ...
    # Validate results
    assert result_cpu.shape == expected.shape, f"Shape mismatch: {result_cpu.shape} vs {expected.shape}"
...
def test_specialized_paged_attn_kernel():
    ...
    q = torch.randn(BATCH_SIZE, NUM_Q_HEADS, SEQ_LEN, HEAD_SIZE, 
                    dtype=torch.float16, device="spyre")
    print(f"Query shape: {q.shape}")
    
    # Create K and V page caches: [NUM_PAGES, NUM_KV_HEADS, BLOCK_SIZE, HEAD_SIZE]
    k_pages = torch.randn(NUM_PAGES, NUM_KV_HEADS, BLOCK_SIZE, HEAD_SIZE,
                          dtype=torch.float16, device="spyre")
    v_pages = torch.randn(NUM_PAGES, NUM_KV_HEADS, BLOCK_SIZE, HEAD_SIZE,
                          dtype=torch.float16, device="spyre")
    ...
    for i in range(NUM_BLOCKS):
        ...
        mask = torch.zeros(BATCH_SIZE, NUM_Q_HEADS, SEQ_LEN, BLOCK_SIZE,
                          dtype=torch.float16, device="spyre")
