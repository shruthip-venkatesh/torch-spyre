...
def compute_index_tensor_layout(
    ...
    logger.debug(f"Index tensor {index_tensor_idx}: all_dims from iteration_space={all_dims}")
    ...
    if value_tensor_idx is not None:
        ...
        logger.warning(f"Could not find value tensor for index tensor {index_tensor_idx}")
        return [], None
...
def create_sdsc_arg_for_tensor(
    ...
    from torch_spyre._inductor.constants import SEGMENT_OFFSETS, MATMUL_LAYOUT_LABELS, LAYOUT_LABELS
    ...
    if tensor_idx in index_tensor_layouts:
        ...
        if is_value_tensor and has_indirect_access:
            ...
            dim_order, stick_dim = get_device_dim_order_func(arg, symbol_mapping, reverse_for_indirect=False)
            # For output tensors in indirect access (gather), reverse the non-stick dimensions
            if should_reverse_output_dims_for_indirect(tensor_idx, len(op_spec.args), has_indirect_access, len(dim_order)):
                non_stick_dims = dim_order[:-1]
                non_stick_dims.reverse()
                dim_order = non_stick_dims + [dim_order[-1]]
                logger.debug(f"Tensor {tensor_idx}: Reversed non-stick dims for indirect access output, new dim_order={dim_order}")
    ...
    if op_stick_dim is None and tensor_idx not in index_args:
        ...
        logger.debug(f"Tensor {tensor_idx}: Added stick_dim={stick_dim}, new dim_order={dim_order}")
    ...
    is_value_or_index_tensor = (
        tensor_idx in index_args or
        tensor_idx in [t.related_value_tensor_idx for t in op_spec.args if t.is_index_tensor]
    )
    ...
    for dim in dim_order:
        ...
        if is_value_tensor_for_indirect:
            ...
            if should_zero_stride_offset(tensor_idx, dim, stick_dim, index_active_dims, is_value_tensor_for_indirect):
                strides[dim] = 0
                offsets[dim] = 0
                logger.debug(
            ...
            if tensor_idx in index_args:
                max_dim_sizes[dim] = compute_index_tensor_max_dim_size(dim, related_value_positive_dims)
                logger.debug(
    ...
    if should_use_kernel_idx_layout(tensor_idx, index_args):
        ...
        logger.debug(f"Tensor {tensor_idx}: About to create KERNEL_IDX layout with dim_order={dim_order}, effective_stick={effective_stick}, scales.keys()={list(scales.keys())}")
        ...
        indirect_label = get_indirect_access_layout_label(
            tensor_idx, is_value_tensor, tensor_idx == len(op_spec.args) - 1, has_indirect_access
        )
        ...
        logger.debug(f"Tensor {tensor_idx}: Assigned {label} layout, dtype={arg.device_dtype.name}, stick_size={arg.device_dtype.elems_per_stick()}")
