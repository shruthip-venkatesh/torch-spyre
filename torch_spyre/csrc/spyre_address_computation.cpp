...
at::Tensor compute_addresses_from_indices(
  ...
  int64_t ndim =
      indices_shape[indices_shape.size() - 1];  // Number of dimensions

  // Validate that ndim matches device_size
  TORCH_CHECK(ndim == static_cast<int64_t>(device_size.size()),
              "Index dimension mismatch: indices have ", ndim,
              " dimensions, "
              "but device_size has ",
              device_size.size(), " dimensions");

  TORCH_CHECK(ndim == static_cast<int64_t>(device_stride.size()),
              "Stride dimension mismatch: indices have ", ndim,
              " dimensions, "
              "but device_stride has ",
              device_stride.size(), " dimensions");
  ...
  auto addresses =
      at::zeros({num_indices}, at::TensorOptions().dtype(at::kFloat));
  ...
  for (int64_t i = 0; i < num_indices; ++i) {
    ...
    for (int64_t dim = 0; dim < ndim; ++dim) {
      ...
      TORCH_CHECK(coord >= 0 && coord < device_size[dim], "Index ", coord,
    ...
    TORCH_CHECK(byte_address % STICK_SIZE_BYTES == 0, "Address ", byte_address,
                " is not stick-aligned (must be multiple of ", STICK_SIZE_BYTES,
                " bytes). Element offset: ", element_offset,
...
at::Tensor compute_stick_addresses_from_rows(const at::Tensor& row_indices,
  ...
  TORCH_CHECK(indices_shape.size() == 2, "Row indices must be 2D tensor, got ",
  ...
  for (int64_t group = 0; group < num_groups; ++group) {
    ...
    for (int64_t idx = 0; idx < rows_to_gather; ++idx) {
      ...
      TORCH_CHECK(row_in_group >= 0 && row_in_group < rows_per_group,
                  "Row index ", row_in_group, " out of bounds for group ",
                  group, " (rows_per_group: ", rows_per_group, ")");
      ...
      TORCH_CHECK(absolute_byte_address % STICK_SIZE_BYTES == 0, "Address ",
                  absolute_byte_address, " for row ", absolute_row,
                  " is not stick-aligned (must be multiple of ",
...
at::Tensor indices_to_addresses_2d(const at::Tensor& logical_indices,
  ...
  TORCH_CHECK(value_shape.size() == 2,
              "value_tensor must be 2D [IN, OUT], got ", value_shape.size(),
              "D");
  ...
  for (int64_t i = 0; i < num_indices; ++i) {
    ...
    // Validate row index is within bounds
    TORCH_CHECK(row_index >= 0 && row_index < in_dim, "Row index ", row_index,
                " out of bounds for dimension IN=", in_dim);
    ...
    TORCH_CHECK(byte_address % STICK_SIZE_BYTES == 0, "Address ", byte_address,
                " for row ", row_index,
                " is not stick-aligned (must be multiple of ", STICK_SIZE_BYTES,
                " bytes). ", "Row stride: ", row_stride_bytes, " bytes");
